...
def equal_cost_backtrace(parent: dict, node_cost: dict, current: Node) -> set[Point]:
    ...
    while current := parent.get(current):
        trace.add(current.p)
        # Check for equal cost paths and backtrace recursively
...
def a_star() -> tuple[int, int]:
    """
    Simple A* algorithm to find the shortest path from start to end.
    Returns the lowest cost and the number of nodes in the optimal path.
    """
    ...
    while len(pending):
        ...
        # Scan in all directions (right, down, left, up)
