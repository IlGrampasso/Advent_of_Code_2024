...
# Custom dictionary to handle registry values
...
# Function to get the combo value
    ...
    while pointer < len(program):
        ...
        match opcode:
            case Opcode.ADV | Opcode.BDV | Opcode.CDV:
                ...
                registries[target] = registries[RegistryType.A] // (2 ** combo(operand, registries))
            ...
            case Opcode.BXC:
                registries[RegistryType.B] = registries[RegistryType.B] ^ registries[RegistryType.C]
...
def part_1() -> str:
    registries, program = parse("input1.txt")
    return ",".join(map(str, evaluate(registries, program)))

# Part 2: Find the lowest positive initial value for register A
def part_2() -> int:
    registries, program = parse("input1.txt")
...
print("Part 2 Lowest Initial Value for Register A:", part_2())
